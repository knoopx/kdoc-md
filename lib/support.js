...
function parse_name(str, data) {
    ...
    if (quotedGroups.length > 1 && quotedGroups[0] === '' && quotedGroups.length % 2 === 1) {
        ...
        while (pos < str.length) {
            brackets += (str[pos].match(/[([{<]/) ? 1 : (str[pos].match(/[)\]}>]/) ? -1 : 0))
            name += str[pos]
            pos++
...
function parseSignature(decl) {
    ...
    if (matches) {
        return Array.from(matches[0].matchAll(/(\<[^\>]*\>|[A-Za-z0-9\.\@]+(\<[^\>]*\>)?)+/g)).map(x => x[0].trim())
    }
    return []
}
...
function generateMarkdown(data) {
    ...
    data.forEach(match => {
        if (match.type.includes("class"))
            markdown += "## ";
        else markdown += "### ";

        markdown += match.name + (match.type.length > 0 ? " `" + match.type.join("` `") + "`" : "") + "\n\n";
        markdown += parseTags(match.description) + "\n\n";

        if (match.tags.length) {
            markdown += table([
                ["Type", "Name", "Description"],
                ...match.tags.map(tag => ([tag.tag, "`" + tag.name + "`", parseTags(tag.description)]))
            ]) + "\n"
        }
